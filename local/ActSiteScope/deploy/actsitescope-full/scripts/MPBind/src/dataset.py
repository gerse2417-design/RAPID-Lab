...
def update_struct(struct, mask):
    ...
    # To check whether two different residues correspond to one resid:
    current_pos = -1000
    rnamenew = []
    tname = ""
    
    # Ensure iterability
    resnames = struct['resname']
    resids = struct['resid']
    if np.isscalar(resnames): resnames = [resnames]
    if np.isscalar(resids): resids = [resids]
    
    for rname, rid in zip(resnames, resids):
        if rid != current_pos:
            current_pos = rid
            tname = rname
