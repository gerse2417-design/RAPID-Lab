...
# Python-native DSSP replacements
try:
    import pydssp
except ImportError:
    pydssp = None
...
def get_python_dssp(pdb_file_path, seq):
    """
    Python-native replacement for mkdssp using pydssp and Biopython.
    """
    if pydssp is None or PDBParser is None:
        return None
        
    try:
        # 1. Secondary Structure (SS) using pydssp
        with open(pdb_file_path, "r") as f:
            lines = f.readlines()
            X_coords = get_pdb_xyz(lines) # [L, 5, 3] (N, CA, C, O, R)
            
        # [STABILITY GUARD] Skip pydssp if L is too short or 0
        if X_coords is None or X_coords.shape[0] < 3:
             print(f"[*] Warning: X_coords too short for pydssp (L={len(X_coords) if X_coords is not None else 0})")
             return np.zeros((len(seq), 8))

        X_pt = torch.tensor(X_coords, dtype=torch.float32).unsqueeze(0) # [1, L, 5, 3]
        # pydssp.assign expects N, CA, C, O (indices 0, 1, 2, 3)
        # It's an array of strings like [['-', 'E', 'H', ...]]
        ss_labels = pydssp.assign(X_pt[:, :, :4, :]) # [1, L] array of characters
        
        # Mapping characters to HBEGITSC (8-state)
        ss_map = {'H': 0, 'B': 1, 'E': 2, 'G': 3, 'I': 4, 'T': 5, 'S': 6, 'C': 7, '-': 7, 'L': 7}
        L_pdb = X_pt.shape[1]
        ss_matrix = np.zeros((L_pdb, 8))
        
        for i in range(L_pdb):
            char = str(ss_labels[0][i]).upper()
            idx = ss_map.get(char, 7) # Default to Coil (7)
            ss_matrix[i, idx] = 1.0
        
        # 2. Solvent Accessibility (ASA) using Biopython
        parser = PDBParser(QUIET=True)
        structure = parser.get_structure("protein", pdb_file_path)
        sr = ShrakeRupley()
        sr.compute(structure, level="R")
        
        # Mapping back to the sequence
        # We need to ensure we follow the same residue order as seq
        # rASA_std same as in process_structure.py
        aa_type = "ACDEFGHIKLMNPQRSTVWY"
        rASA_std = [115, 135, 150, 190, 210, 75, 195, 175, 200, 170,
                    185, 160, 145, 180, 225, 115, 140, 155, 255, 230]
        
        rsas = []
        # Get all residues from the first model, first chain
        # NOTE: MPBind inference.py usually processes single-chain PDBs here
        model = structure[0]
        residues = [r for r in model.get_residues() if r.id[0] == " "] # Filter out hetero/water
        
        # Simple mapping by index (assuming PDB matches seq)
        # For more complex cases, we should align or use residue IDs
        for i, res in enumerate(residues):
            if i >= len(seq): break
            aa = seq[i]
            asa = getattr(res, "sasa", 0.0)
            idx = aa_type.find(aa)
            if idx != -1:
                rsa = min(1.0, asa / rASA_std[idx])
            else:
                rsa = 0.0
            rsas.append(rsa)
            
        # Pad if residues count < seq length
        if len(rsas) < len(seq):
            rsas.extend([0.0] * (len(seq) - len(rsas)))
            
        # Combine [RSA (1), SS (8)] -> [9]
        dssp_matrix = []
        for i in range(len(seq)):
            feat = np.concatenate(([rsas[i]], ss_matrix[i]))
            dssp_matrix.append(feat)
            
        return dssp_matrix
    except Exception as e:
        print(f"ERROR in get_python_dssp: {e}")
        return None
def manual_radius_graph(x, r, loop=False, max_num_neighbors=32):
    """Pure-torch manual implementation of radius_graph."""
    with torch.no_grad():
        ...
        # Slower but robust neighbor limiting
        if max_num_neighbors < x.size(0):
            # Sort distances to find top K nearest neighbors
            _, indices = torch.sort(dist, dim=-1)
            # Create a mask for top K (including those within radius r)
            # This is slightly different from PyG's radius_graph which might be more efficient
            new_mask = torch.zeros_like(mask)
            for i in range(x.size(0)):
                count = 0
                for j in range(x.size(0)):
                    idx = indices[i, j]
                    if (idx != i or loop) and dist[i, idx] <= r:
                        new_mask[i, idx] = True
                        count += 1
...
def extract_feat(ID_list, seq_list, outpath, gpu, script_path = script_path, ProtTrans_path = ProtTrans_path):
    ...
    for name in ['DSSP', 'ProtTrans', 'Atomic', 'ProsT52AA', 'chain']:
    ...
    print(f"[*] Processing Structural Data (L={len(seq_list[0])}) for {len(ID_list)} atoms...")
    for ID, seq in zip(ID_list, seq_list):
        L = len(seq)
        with open(outpath + "/chain/" + ID + ".pdb", "r") as f:
            X = get_pdb_xyz(f.readlines()) # [L_atoms, 5, 3]
            
        # Ensure length matches sequence
        if X.shape[0] < L:
            # Pad with zeros
            padding = np.zeros((L - X.shape[0], 5, 3), dtype=np.float32)
            X = np.concatenate([X, padding], axis=0)
        elif X.shape[0] > L:
            # Truncate
            X = X[:L]
            
        torch.save(torch.tensor(X, dtype = torch.float32), outpath + "/chain/" + ID + '.tensor')


    print(f"[*] Extracting DSSP Structural Features (Python-Native) for {len(ID_list)} sequences...")
    for i in tqdm(range(len(ID_list))):
        ID = ID_list[i]
        seq = seq_list[i]
        L = len(seq)
        pdb_file_path = os.path.join(outpath, "chain", f"{ID}.pdb")
        
        if not os.path.exists(pdb_file_path):
            print(f"ERROR: PDB file not found at {pdb_file_path}. Skipping.")
            torch.save(torch.zeros((L, 9)), "{}/DSSP/{}.tensor".format(outpath, ID))
            continue

        # Use Python-native DSSP
        dssp_matrix = get_python_dssp(pdb_file_path, seq)
        
        if dssp_matrix is None:
            print(f"WARNING: Python-native DSSP failed for {ID}. Using zero fallback.")
            torch.save(torch.zeros((L, 9)), "{}/DSSP/{}.tensor".format(outpath, ID))
        else:
            # Ensure length
            if len(dssp_matrix) != L:
                print(f"WARNING: DSSP length mismatch ({len(dssp_matrix)} vs {L}). Adjusting...")
                if len(dssp_matrix) < L:
                    dssp_matrix = np.concatenate([dssp_matrix, np.zeros((L - len(dssp_matrix), 9))], axis=0)
                else:
                    dssp_matrix = np.array(dssp_matrix)[:L]
...
def extract_Alphafold(ID_list, seq_list, outpath, gpu, filename = None, start = 83153, end = 120000, resume = True):
    ...
    # Min-Max is used to normalize the sequence embeddding from ProtTrans
    ptrans_path = outpath
    Min_protrans = torch.tensor(np.load(os.path.join(script_path, "Min_ProtTrans_repr.npy")), dtype = torch.float32)
    Max_protrans = torch.tensor(np.load(os.path.join(script_path, "Max_ProtTrans_repr.npy")), dtype = torch.float32)
    ...
    print("Extracting DSSP features (Python-Native)...")
   
    for i in tqdm(range(len(ID_list))):
        ID = ID_list[i]
        seq = seq_list[i]
        pdb_file_path = f"{outpath}/pdb/{ID}.pdb"
        
        dssp_matrix = get_python_dssp(pdb_file_path, seq)
        if dssp_matrix is None:
             torch.save(torch.zeros((len(seq), 9)), "{}/DSSP/{}.tensor".format(outpath, ID))
        else:
            torch.save(torch.tensor(np.array(dssp_matrix), dtype = torch.float32), "{}/DSSP/{}.tensor".format(outpath, ID))
...
class ProteinGraphDataset(data.Dataset):
    ...
    def __update_selection(self):
        ...
        for pp in self.IDs:
            ...
            if X.shape[0] == dssp_feat.shape[0] and X.shape[0] > 0:
                mask.append(True)
            else:
                mask.append(False)
    ...
    def _featurize_graph(self, idx):
        ...
        with torch.no_grad():
            ...
            edge_index = manual_radius_graph(X_ca, r=self.radius, loop=True, max_num_neighbors = 100)
...
def _get_direction_orientation(X, edge_index): # N, CA, C, O, R
    ...
    if t.shape[0] == 0:
        node_direction = torch.zeros((0, 12), device=X.device)
    else:
    ...
    if t.shape[0] == 0:
        edge_direction_ji = torch.zeros((0, 15), device=X.device)
    else:
    ...
    if t.shape[0] == 0:
        edge_direction_ij = torch.zeros((0, 15), device=X.device)
    else:
        edge_direction_ij = torch.matmul(t, local_frame[node_j]).reshape(t.shape[0], -1) # [E, 5 * 3]
    
    edge_direction = torch.cat([edge_direction_ji, edge_direction_ij], dim = -1) # [E, 30]
