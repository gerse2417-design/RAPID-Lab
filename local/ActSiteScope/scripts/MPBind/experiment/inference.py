...
def inference(pdb_path, model_path, weight_name, version = "", predict_path = ""):
    ...
    # find pdb files and ignore already predicted ones or intermediate ones
    # Filter: Only keep files in the root pdb_path, ignore 'chain' folder or files already split
    all_files = glob(os.path.join(pdb_path, "*.pdb*"))
    pdb_filepaths = []
    for f in all_files:
        basename = os.path.basename(f)
        # Final strict filter: Original files should not have underscores in their main name if possible,
        # or at least they should not be in the 'chain/' subfolder.
        # Most importantly, if we already have 1m40_A_0.pdb, it's a split chain.
        # Original: 1m40.pdb1  (count("_") = 0)
        # Split: 1m40_A_0.pdb (count("_") = 2)
        if "chain" in f or basename.count("_") >= 2: 
            continue
        # Also skip already saved results in 'results' folder if they were copied there
        if "results" in f:
            continue
        pdb_filepaths.append(f)

    if not pdb_filepaths:
        # Fallback to all if filtering was too aggressive
        pdb_filepaths = [f for f in all_files if os.path.isfile(f)]
        
    print(f"[*] Found {len(pdb_filepaths)} original PDB files to analyze.")
    ...
    print(f"[*] Analyzing protein structures in {pdb_path}...")
    for subunits, filepath in tqdm(dataset):
        ...
        for cid0 in subunits:
            ...
            print(f"\n[Step 1/3] Processing: {pdb_filename} (Chain {cid0})")
            ...
            # [STABILITY GUARD] Skip if sequence is too short (ProtTrans/pydssp often fail on L < 3) 
            if len(new_seq) < 5:
                print(f"[*] Skipping Chain {cid0} - Sequence too short (L={len(new_seq)})")
                continue
            ...
            print(f"[Step 2/3] Extracting sequence and structural features...")
            ...
            print(f"[Step 3/3] Running AI model prediction...")
            dataloader = set_graphDataloader(ID_list = [pdb_id], feats_out = pdb_path, batch = 1, shuf = False, drop = False, df_chunk = None, version = version)
            
            if len(dataloader) == 0:
                print(f"WARNING: Skipping {pdb_id} - could not generate valid graph features.")
                continue
            ...
            if num_nodes == 0 or node_allft.shape[0] == 0:
                print(f"WARNING: Skipping {pdb_id} - empty feature tensors.")
                continue
            ...
            try:
                ...
                p_full = pt.sigmoid(z).detach()
                print(f"[*] Prediction completed for {pdb_id}")
            except Exception as e:
                print(f"ERROR in model.forward for {pdb_id}: {e}")
                continue
            ...
            for btype in args.binding_type:
                ...
                # Use the full pdb_id instead of truncating it
                filepath = f"{predict_path}/{pdb_id}"
                output_filepath = filepath +'_i{}_v{}.pdb'.format(btype, version)
                print(f"[*] Saving prediction to {output_filepath}")
                save_pdb({cid0: structure}, output_filepath)
                results_saved += 1
            
            if results_saved == 0:
                print(f"WARNING: No prediction results were saved for {pdb_id}")
...
if __name__ == '__main__':
    ...
    if os.path.isabs(args.input):
        rawpdb_path = args.input
    else:
        rawpdb_path = os.path.abspath(os.path.join(str(root), args.input))
    ...
    if os.path.isabs(args.output):
        ...
        result_path = os.path.abspath(os.path.join(rawpdb_path, args.output))
