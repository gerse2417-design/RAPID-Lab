...
def get_pdb_DF(file_path):
    ...
    # Full list of columns used in PDBResidueFeature
    columns = ['ID', 'atom', 'atom_type', 'res', 'res_id', 'xyz', 'B_factor', 'mass', 'is_sidechain', 'charge', 'num_H', 'ring']
    pdb_res = pd.DataFrame(columns=columns)
    ...
    print(f"DEBUG: Reading PDB file: {file_path}")
    count_total = 0
    for line in pdb_file:
        count_total += 1
        if line.startswith('ATOM') or line.startswith('HETATM'):
            # print(f"DEBUG: Processing ATOM line: {line[:30]}...")
            # Try to get atom type from columns 77-78, fallback to atom name
            atom_type = line[76:78].strip().upper()
            if not atom_type:
                atom_name = line[12:16].strip()
                # Use first letter as element (simplified but common)
                atom_type = "".join([c for c in atom_name if c.isalpha()])[:1].upper()
            
            if atom_type not in Relative_atomic_mass:
                # If still not found, ignore but don't skip line to avoid empty DF if possible
                atom_type = 'C' # Default fallback
            ...
            atom_name = line[12:16].strip()
            if atom_name not in ['N','CA','C','O','H']:
                is_sidechain = 1
            else:
                is_sidechain = 0
                
            res_3 = line[17:20].strip().upper()
            res = res_dict.get(res_3, 'A') # Default to Ala if unknown
            
            try:
                atom_fea = atom_fea_dict.get(res, {}).get(atom_name, [0.5, 0.5, 0.5])
            except:
            ...
            try:
                ...
                continue # Skip if coord invalid

            mass_val = Relative_atomic_mass.get(atom_type, 12.0)
            
            tmps = pd.DataFrame(
                {'ID': [atom_count], 'atom':[atom_name],'atom_type':[atom_type], 
                 'res': [res], 'res_id': [res_pdb_id],
                 'xyz': [xyz],
                 'B_factor': [bfactor],'mass':[mass_val],'is_sidechain':[is_sidechain],
                 'charge':[atom_fea[0]],'num_H':[atom_fea[1]],'ring':[atom_fea[2]]}) 
                 
            if res_pdb_id not in res_id_list:
                res_id_list.append(res_pdb_id)
                
            pdb_res = pd.concat([pdb_res, tmps], ignore_index=True)
            
        if line.startswith('TER') or line.startswith('END'):
            # Don't break immediately if there are multiple chains and we want all?
            # Actually inference.py splits chains, so there's only one chain per file.
            break

    pdb_file.close()
    if len(pdb_res) == 0:
        print(f"ERROR: No atoms were parsed! Check line matching and Relative_atomic_mass keys.")
        # Print a sample line if possible
    print(f"DEBUG: Parsed {len(pdb_res)} atoms from {count_total} lines")
...
def PDBFeature(query_id,PDB_chain_dir,results_dir):
    ...
    # Always regenerate to avoid stale pickle issues
...
def PDBResidueFeature(query_path,query_id,pssm = None,hhm = None,dssp = None):
    ...
    if len(B_factor) == 0:
        print(f"WARNING: No atoms in {query_id} for B_factor normalization. Returning zero features.")
        res_atom_feas = np.zeros((len(res_id_list), 6))
    ...
    for i, res_id in enumerate(res_id_list):
        ...
        if len(res_atom_df) == 0:
            # Skip residues with no atoms to avoid NaN/Zero division
            continue
        ...
        res_atom_df_list.append(res_atom_feas_i)
        atom_begin += atom_num
    
    if len(res_atom_df_list) == 0:
        print(f"WARNING: No valid residue features for {query_id}")
        return np.zeros((len(res_id_list), 6))
        
    res_atom_feas = np.concatenate(res_atom_df_list, axis=0)
